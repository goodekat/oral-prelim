...
No accepted definitions for explainability and interpretability .medium[`r Citep(bib, c("gilpin:2018", "lipton:2016", "molnar:2019", "montavon:2017", "murdoch:2019"))`]  

My definitions .medium[(implicitly used by `r Citet(bib, "rudin:2018")` and `r Citet(bib, "ribeiro:2016")`)]:
...
Claims made about LIME `r Citep(bib, "ribeiro:2016")`:
...
- .red[Axes]: simulated data features ( $x'_1$ and $x'_2$ )
...
sine_lime_default %>%
  ...
  labs(x = "Random Forest Prediction",
       ...
       color = "Explainer \nModel \nR2",
       title = "Complex and Explainer Model Prediction Comparison Plot") +
...
.left-desc[
**Quantile Grid Prediction Comparison Plot**
...
ggplot() +
  ...
  labs(x = TeX("$x_1$"),
       ...
       fill = "Average \nExplainer \nModel \nProbability \nfor 'blue'",
       color = "Random \nForest \nProbability \nfor 'blue'",
       title = "Quantile Grid Prediction Comparison Plot") +
  guides(shape = guide_legend(order = 1),
...
**Average Fidelity** `r Citep(bib, "ribeiro:2016")`: 
...
- Since LIME did not provide reasonable explanations for the bullet matching random forest from the study by `r Citep(bib, "hare:2016")`, it is still of interest to visualize the random forest to gain insight into the predictions. 
